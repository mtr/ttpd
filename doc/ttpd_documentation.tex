...
\coal{TAD} software.  The software package contains two programs,
named \code{ttpd} and \code{ttpc}.  These programs use a common
\coal{API}, which is also documented herein.
...
characterizes \coa{TTPD} and \coa{TAD}.
...
\begin{itemize}
\item The design of \coa{TTPD} allows a computer to serve multiple
  \coa{TUC} requests in parallel.  This is ensured through the use of
  the pool of \co{TUC-threads} where each thread controls an
  externally running \coa{TUC} process.  This means that if more
...
\item By combining the use of \coP{socket} with \coP{tread} (could
  have used \co{forked processes} but they take longer to create) the
  \coa{daemon} is able to stack up incoming requests and serve them as
...
\item The use of \coP{socket} ensure that \emph{if} the \co{daemon}
  ...
  \coa{TAD} alerts.  They will be sent out as soon as possible after
...
\item The daemon tries to cope with non-perfect conditions in a
  ...
  \item If one of the encapsulated \coa{TUC} processes die
    unexpectedly, the daemon will start a replacement process
    immediately (this constitutes a built-in \co{watch dog} feature).

  \item All alerts that are registered by the \coa{TAD} is immediately
  ...
  \item Because the \co{protocol} for \co{communication} with the
    \coa{SMS} \co{message switch} defines the communication as
...
\item Since the \co{SMS message switch} \co{communication protocol}
  uses \coa{XML}-based headers, the daemon parses these with the
  default \con{Python} \co{SAX-parser} (usually an \co{expat parser}).
  This ensures a robust handling of incoming messages and an automatic
  check for \coa{XML} compliance.

\item The \coa{TTPD} is designed to handle random connections to its
...
included here to make the \coa{API} documentation meaningful.
